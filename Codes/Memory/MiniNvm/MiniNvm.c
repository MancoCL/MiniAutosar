...
*   作者         @: Manco
************************************************************************************************
 *   描述         @: MiniNvm 实现：以队列缓存单块读写请求并逐个启动 MiniFee 异步作业，
 *                   ReadAll/WriteAll 走一次性批量路径；周期查询结果并完成数据搬运与总体结果聚合。
*
************************************************************************************************
*   限制         @: 无
*
 ...
 *
 * MiniNvm_MainFunction 每拍只做一件事：有当前作业则查询并收尾，否则从队列取下一项启动。
 * 注意：MiniNvm 不调用 MiniFee_MainFunction。外部必须按
...
/* 队列条目：只保存单块请求元数据（读写数据统一走上下文共享 DataBuf） */
typedef struct
{
...
/* 判断待写入内容（前 length 字节 + 其后补零至块长）是否与目标块 RAM buffer 完全一致，
...
/* 由 MiniNvm 块号（1..N）取描述符；越界返回 NULL */
...
/* 由 MiniFee 块号（0..N-1）取描述符；越界返回 NULL */
...
/* MiniFee 一次性读回调：把每条记录数据拷入对应块 RAM buffer。
...
/* MiniFee 一次性写取数回调：返回该块 RAM buffer 指针（待落盘）；非脏块返回 NULL 跳过。 */
...
/* 判断指定块是否已有挂起请求（当前作业或队列中），用于拒绝同块重复请求 */
...
/* 结束当前单块作业：写回单块结果，写成功时清脏标记，最后清空当前作业上下文。 */
...
/* 从队首取出一个请求并启动 MiniFee 作业：读出队首并前移头指针，记录当前作业上下文，
...
/* 收尾一次性读（MiniFee_ReadAll）：全部块结果与多块总体结果统一置位 */
...
/* 收尾一次性写（MiniFee_WriteAll）：成功的脏块清脏并置 OK，失败的保留脏并置 NOT_OK；
 * 多块总体结果置 result。 */
...
/* 初始化：初始化 MiniFee，校验块描述符配置，复位队列、结果与脏标记。
...
/* 请求异步读取指定块到 dstPtr：校验状态/指针/是否已有同块挂起请求后入队（块号合法性由入队校验）。
...
/* 请求异步写入指定块：校验后与当前 RAM buffer 比对，一致则直接返回（内容去重，不置脏/不入队）；
...
/* 一次性读取全部块到各自 RAM buffer：先清零（Flash 中无记录的块保持默认全零），
...
/* 一次性写入全部脏块到非易失存储：先统计有无脏块（无则直接 OK），再由 MiniFee_WriteAll
...
/* 取消当前作业与队列中所有请求：若 MiniFee 正 BUSY 则先取消它；相关单块结果置 NOT_OK，
...
/* 查询指定单块（blockId 从 1 开始）最近一次请求结果 */
...
/* 获取指定块（MiniFee 块号，从 0 开始）的 RAM buffer 地址；不触发 Flash 作业 */
...
/* 从指定块的 RAM buffer 同步读取 length 字节到 dstPtr；length 不得超过该块长度，不触发 Flash 作业 */
...
/* 同步写入指定块的 RAM buffer：写入 length 字节，剩余部分清零；仅当与原数据不一致时才置脏。
