...
- **Flash 驱动 mock**：[test/FlashDrv_Stub.c](file:///d:/WorkSpace/MiniAutosar/test/FlashDrv_Stub.c) 用 RAM 数组模拟 Flash（总容量 16KB），强制 1→0 写约束，擦除置 0xFF；**写操作强制按 `MINIFEE_PAGE_SIZE` 对齐且长度为其整数倍**（模拟物理最小编程单元），违反返回 FLASH_ERR_PARAM；`writeGranularity = MINIFEE_PAGE_SIZE`。`FlashDrv_Init` 不擦除（保留内容跨"重启"），由 `FlashDrv_Stub_Reset` 负责全新首启。
- **故障注入**：
  - `FlashDrv_Stub_FailStatusWrites(n)`：令接下来 n 次**提交页写**失败（识别约定：整页写且首 2 字节为 0x55 0x55，即提交页 status=VALID 特征），模拟提交前掉电。
  - `FlashDrv_Stub_FailAnyWrites(n)`：任意写失败。
  - `FlashDrv_Stub_FailErases(n)`：擦除失败。
  - `FlashDrv_Stub_SetByte/GetByte`：绕过约束注入损坏/检视。
- **掉电注入**：通过 `FailStatusWrites(1)` 在提交页写入步失败并再次 `MiniFee_Init` 模拟重启恢复。
...
测试块表：8 块、size 均 128（运行时传数组给 `MiniFee_Init`）。

| 编号 | 前置 | 步骤 | 预期 | 覆盖 P0 |
|---|---|---|---|---|
| TC-F-01 | 全 0xFF | Reset+Init(sizes,8)；ReadBlock(0) | Init OK；NOT_FOUND；属性正确（GetWritePageSize=PAGE_SIZE、GetBlockDataSize(0)=128、numBlocks=8） | #9,#12 |
...
| TC-F-03 | 空 Flash | WriteBlock(0,v1)→WriteBlock(0,v2)→Read | v2（旧槽靠 seq 竞争，读最新） | #5,#6 |
| TC-F-04 | 空 Flash | 写 block1/2 各一次 + block0 反复写填满 active(1024 页)→再写 block0 触发 GC→读 block0/1/2 | GC OK；三块最新值保持 | #6,#7 |
| TC-F-05 | 写过 block1 | 破坏块 1 槽数据区首字节（槽 base+PAGE_SIZE）→Init→ReadBlock(1) | ERR_CRC | #10 |
| TC-F-06 | 写过 block2 | EraseBlock(2)（写墓碑）→ReadBlock→再 EraseBlock | NOT_FOUND；幂等 OK（不再写） | #11 |
| TC-F-07 | 写过 block3 v1 | FailStatusWrites(1)→WriteBlock(3,v2) 失败（提交页写失败）→Init→ReadBlock(3) | 返回 FLASH 错；恢复读回 v1（丢最新槽） | #9 |
| TC-F-08 | Init | 非法参数（越界/NULL/len 过大） | ERR_PARAM | — |
| TC-F-09 | Init | Init(NULL,8)/Init(sizes,0)/Init(sizes,MAX+1)/size 非 PAGE_SIZE 整数倍 | ERR_PARAM | — |

> 容量约束 `ΣblockPages+max <= pagesPerCluster` 在当前配置（8 块×128B、1024 页/cluster）下无法触发，实现中作防御性校验保留。
...
| TC-N-05 | 写过块2+WriteAll | 破坏数据（块 2 槽数据区首字节=槽 base+PAGE_SIZE=8）→重启 Init+ReadAll→GetErrorStatus(2)→ReadBlock(2) | ERR_CRC；E_NOT_OK | #10,#11 |
