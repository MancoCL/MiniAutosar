...
- 注释一律中文，仅用 C 风格 `/* */` 与 `/** */`，禁用 `//`，不得嵌套注释。
- 文件头模板：`\file` 文件名；`\brief` 一句话职责；`\details` 分层定位、关键约束与对应设计文档（`docs/*.md`）；`\req` 模块级需求追溯。
- 函数注释模板（头文件中的对外 API 必须全项）：
  - `\brief` 一句话语义；`\details` 行为/边界/掉电语义（按需）；
  - `\param[in|out]` 逐参数说明（含取值范围与是否可空）；
  - `\return` 逐返回值含义；
  - `\req` 需求追溯：`P0 #N`（docs/06）或 `TC-F-xx`/`TC-N-xx`（docs/05）。
- 实现文件（`.c`）中的对外 API 只注实现要点（`\brief`/`\details`/`\req`），接口契约统一放在对应头文件；静态内部函数需完整 `\param`/`\return`。
- 宏、类型、模块全局数据用 `/** */` 文档注释；配置宏保留【假设】占位标记。
