 ...
 *
 * 所有量化参数均为【假设】占位默认值，集成时必须按目标 Flash 属性对齐填写。
...
/** CRC 算法选择：0=CRC16, 1=CRC32 */
#define MINIFEE_CRC_TYPE_CRC32      (1u)
#define MINIFEE_CRC_TYPE            MINIFEE_CRC_TYPE_CRC32
...
/** CRC16 多项式。【假设】（当 MINIFEE_CRC_TYPE 选 CRC16 时使用） */
...
#define MINIFEE_STATUS_ERASED       ((uint16)0xFFFFu)  /* 空 */
#define MINIFEE_STATUS_VALID        ((uint16)0x5555u)  /* 已提交有效 */
#define MINIFEE_STATUS_INVALID      ((uint16)0x0000u)  /* 已作废 */

/* ---- GC 触发阈值：当前写 cluster 剩余空闲页 ≤ 此值时，下次写前触发 GC。
 *  0 表示仅在写满时触发。【假设】 */
#define MINIFEE_GC_THRESHOLD        ((uint16)0u)

/* ---- 编译期约束校验 ---- */
/* 实际块数运行时传入，MiniFee_Init 会做运行时容量校验。 */
